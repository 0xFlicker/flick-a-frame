...
import * as importedModule$0 from "./sources/uniswap-v3-base/introspectionSchema";
...
export type MeshContext = UniswapV3BaseTypes.Context & BaseMeshContext;
...
const uniswapV3BaseHandler = new GraphqlHandler({
              ...
              config: {"endpoint":"https://api.thegraph.com/subgraphs/name/messari/uniswap-v3-base"},
              baseDir,
              cache,
              pubsub,
...
const merger = new(BareMerger as any)({
  ...
  return {
    ...
    get documents() {
      return [
      {
        document: SwapsDocument,
        get rawSDL() {
          return printWithCache(SwapsDocument);
        },
        location: 'SwapsDocument.graphql'
      },{
        document: TokenPriceDocument,
        get rawSDL() {
          return printWithCache(TokenPriceDocument);
        },
        location: 'TokenPriceDocument.graphql'
      }
    ];
    },
    fetchFn,
...
export type SwapsQuery = { swapsIn: Array<Pick<Swap, 'id' | 'amountIn' | 'amountInUSD'>> };
...
export type TokenPriceQuery = { token?: Maybe<Pick<Token, 'lastPriceUSD' | 'name' | 'symbol'>> };
...
export const SwapsDocument = gql`
  ...
  swapsIn: swaps(
    ...
    ` as unknown as DocumentNode<SwapsQuery, SwapsQueryVariables>;
export const TokenPriceDocument = gql`
    query TokenPrice($token: ID!) {
  token(id: $token) {
    lastPriceUSD
    name
    symbol
    ...
    ` as unknown as DocumentNode<TokenPriceQuery, TokenPriceQueryVariables>;
...
export function getSdk<C, E>(requester: Requester<C, E>) {
  return {
    Swaps(variables: SwapsQueryVariables, options?: C): Promise<SwapsQuery> {
      return requester<SwapsQuery, SwapsQueryVariables>(SwapsDocument, variables, options) as Promise<SwapsQuery>;
    },
    TokenPrice(variables: TokenPriceQueryVariables, options?: C): Promise<TokenPriceQuery> {
      return requester<TokenPriceQuery, TokenPriceQueryVariables>(TokenPriceDocument, variables, options) as Promise<TokenPriceQuery>;
    }
  };
}
